    ...
    def post(self, request: HttpRequest):
        ...
        if form.is_valid():
            ...
            html_message = render_to_string(
                "accounts/email.html", {"welcome": welcome}
            )
...
class PostListView(LoginMixin, PaginatorMixin, View):
    ...
    def get(self, request: HttpRequest):
        ...
        author_id_list = FollowersModel.objects.filter(
            follower=user
        ...
        if request.user.is_superuser is True:
            ...
            posts = PostModel.objects.filter(
                Q(author_id=request.user.id) | Q(author_id__in=author_id_list)
            ).order_by("-created_on")
...
class EditProfileView(LoginMixin, View):
    ...
    def get(self, request: HttpRequest):
        ...
        if user.is_staff:
            return render(
                request,
                "accounts/edit_admin_profile.html",
                {"form": form, "curr_user": user},
            )
        return render(
            request,
...
class ProfileView(LoginMixin, PaginatorMixin, View):
    ...
    def get(self, request: HttpRequest, profile_id: int):
        ...
        flag = FollowersModel.objects.filter(
            Q(follower=request.user) & Q(user=user.id)
        ).exists()
        follower = FollowersModel.objects.filter(user=user.id).count()
    ...
    def post(self, request: HttpRequest, profile_id: int):
        ...
        if "follow" in request.POST:
            FollowersModel.objects.create(
                follower=request.user, user=user_to_follow
            )
...
class AdminView(LoginMixin, AllowedUsersMixin, View):
    ...
    def get(self, request: HttpRequest):
        ...
        """
        return render(
            request, "accounts/admin.html", {"curr_user": request.user}
        )
